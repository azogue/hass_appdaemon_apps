...
class MotionLights(appapi.AppDaemon):
    ...
    def initialize(self):
        ...
        self._lights_check_off = [l for l in self.args.get('lights_check_off', '').split(',') if len(l) > 0]
        ...
        if pir and motion_light_timeout_slider and self._lights_motion:
            ...
            if motion_light_timeout_slider.startswith('input_slider'):
                ...
                self._motion_light_timeout = int(round(float(motion_light_timeout_slider)))
            ...
            if self._media_player is not None:
                ...
                self.log('MotionLightsConstrain media player "{}" (PIR={})'.format(self._media_player, self._pir))
            ...
            if self._extra_constrain_input_boolean is not None:
                ...
                self.log('MotionLightsConstrain extra "{}" (extra_cond now={})'
                         .format(self._extra_constrain_input_boolean, self._extra_condition))
            self.log('MotionLights [{}] with motion in "{}", with timeout={} s, check_off={}. ---> ACTIVE'
                     .format(self._lights_motion, self._pir, self._motion_light_timeout, self._lights_check_off))
        else:
            self.log('No se inicializa MotionLights, faltan parámetros (req: {})'
                     .format('motion_light_timeout, lights_motion, pir'), level='ERROR')
    ...
    def _light_motion_state(self, entity, attribute, old, new, kwargs):
        self.log('New state light {}, old={}, new={}, kwargs'.format(entity, old, new, kwargs))
        ...
        if self._motion_lights_running and self._extra_condition and not self._media_player_active:
            if (not self._lights_check_off or
                ...
                self.log('NO TURN_OFF MOTION_LIGHTS (other lights in the room are ON={})'
                         .format([self.get_state(l) for l in self._lights_check_off]), LOG_LEVEL)
            self._motion_lights_running = False
